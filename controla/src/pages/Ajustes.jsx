...
import GerenciarContas, { BancoBadge, BANCOS } from '../components/GerenciarContas';
import { useAuth } from '../contexts/AuthContext';
import { getUserPreferences, updateUserPreferences, getContaSaldos } from '../lib/database';
...
const Ajustes = () => {
  ...
  const [contaSaldos, setContaSaldos] = useState([]);
  ...
  const loadPreferences = async () => {
    try {
      ...
      if (data) {
        setPrefs({
          notifVencimento: data.notif_vencimento,
          notifIA:         data.notif_ia,
          whatsapp:        data.whatsapp,
          darkMode:        data.dark_mode,
          biometria:       data.biometria,
          relatorio:       data.relatorio_semanal,
          whatzapRegistered: data.phone || data.whatzap || '',
        });
      }
    } catch (error) {
  ...
  const loadContas = async () => {
    ...
    try {
      const data = await getContaSaldos(user.id);
      setContaSaldos(data);
    } catch (e) {
  ...
  const formatWhatsApp = (value) => {
    ...
    if (numbers.length >= 12) { // Ex: 5511999999999
      return `+${numbers.slice(0, 2)} (${numbers.slice(2, 4)}) ${numbers.slice(4, 9)}-${numbers.slice(9)}`;
    }
    return value;
  };

  const saldoTotal = contaSaldos.reduce((s, c) => s + (c.saldo || 0), 0);

  if (loading) {
    return (
      <main className="pt-24 pb-32 px-6 max-w-7xl mx-auto flex items-center justify-center min-h-[50vh]">
        <div className="w-8 h-8 border-3 border-primary-fixed/30 border-t-primary-fixed rounded-full animate-spin" />
      </main>
    );
  }
  ...
  return (
    <>
          ...
          <div className="lg:col-span-5 space-y-4">
            ...
            <div className="p-6 rounded-xl bg-surface-container-low">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <Icon name="account_balance" className="text-primary-fixed text-[20px]" />
                  <h3 className="font-headline font-bold text-on-surface">Contas Bancárias</h3>
                </div>
                <button
                  onClick={() => setIsContasOpen(true)}
                  className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest text-primary-fixed hover:opacity-80 transition-opacity"
                >
              ...
              {/* Saldo consolidado */}
              <div className="mb-4 p-4 rounded-xl bg-primary-fixed/5 border border-primary-fixed/10">
                <p className="font-label text-[9px] uppercase tracking-widest text-on-surface-variant mb-1">Patrimônio Consolidado</p>
                <p className={`font-headline font-extrabold text-2xl tracking-tight ${saldoTotal >= 0 ? 'text-primary-fixed' : 'text-error'}`}>
              ...
              {loadingContas ? (
              ...
              ) : contaSaldos.length === 0 ? (
                <div className="text-center py-4">
                  <p className="text-on-surface-variant text-xs mb-3">Nenhuma conta cadastrada</p>
                  <button
                    onClick={() => setIsContasOpen(true)}
                    className="h-9 px-4 rounded-xl bg-primary-fixed text-on-primary font-bold text-xs flex items-center gap-1.5 mx-auto hover:brightness-110 transition-all"
                  >
                ...
                <div className="space-y-2">
                  {contaSaldos.map(conta => (
                    <div
                      key={conta.id}
                      className={`flex items-center gap-3 p-3 rounded-xl ${
                        conta.padrao ? 'bg-primary-fixed/10' : 'bg-surface-container'
                      }`}
                    >
                      <BancoBadge
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          <p className="font-body font-semibold text-on-surface text-xs truncate">{conta.nome}</p>
                          {conta.padrao && (
                            <Icon name="star" className="text-primary-fixed text-[12px] flex-shrink-0" />
                          )}
                        </div>
                        <p className="text-on-surface-variant text-[10px]">{conta.banco_nome || conta.tipo}</p>
                      </div>
                      <p className={`font-headline font-bold text-sm flex-shrink-0 ${conta.saldo >= 0 ? 'text-primary-fixed' : 'text-error'}`}>
                  ...
                  <button
                    onClick={() => setIsContasOpen(true)}
                    className="w-full mt-1 py-2.5 rounded-xl border border-dashed border-outline-variant/20 text-on-surface-variant text-[10px] font-bold uppercase tracking-widest hover:bg-on-surface/5 transition-colors flex items-center justify-center gap-2"
                  >
                    <Icon name="add" className="text-sm" />
                    Adicionar Conta
                  </button>
            ...
            <div className="p-6 rounded-xl bg-surface-container-low">
              ...
              <p className="text-on-surface-variant text-xs leading-relaxed mb-4">
                Seu agente IA está ativo e monitorando mensagens financeiras no WhatsApp ({formatWhatsApp(prefs.whatzapRegistered || user?.user_metadata?.whatsapp || user?.phone)}).
      ...
      <AnimatePresence>
        {isContasOpen && (
          <GerenciarContas
            onClose={() => setIsContasOpen(false)}
            onContasChange={loadContas}
          />
        )}
      </AnimatePresence>
    </>
